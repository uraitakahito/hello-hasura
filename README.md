...
```
├── compose.base.yaml            # postgres + graphql-engine の共通定義
├── compose.dev.yaml             # 開発環境差分（cli-migrations + Console + bind mount）
├── compose.prod.yaml            # 本番想定差分（素のイメージ + 安全側フラグ + migrator）
├── .env.example
└── hasura/
...
`compose.base.yaml` 単体では起動しません。必ず `compose.dev.yaml` か `compose.prod.yaml` を `-f` で重ねてください。
